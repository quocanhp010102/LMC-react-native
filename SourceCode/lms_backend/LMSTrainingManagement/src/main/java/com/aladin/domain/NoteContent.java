...
public class NoteContent implements Serializable {
    ...
    @Column(name = "note_content_date")
    @JsonFormat(pattern = "HH:mm")
    private Date noteContentDate;
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JsonIgnoreProperties(value = { "questions", "typeOfExams","noteContents","course" }, allowSetters = true)
    private Exams exams;

    @ManyToOne(fetch = FetchType.LAZY)
    @JsonIgnoreProperties(value = { "sender", "authorities","noteContents" }, allowSetters = true)
    ...
    public Date getNoteContentDate() {
        return noteContentDate;
    }

    public void setNoteContentDate(Date noteContentDate) {
        this.noteContentDate = noteContentDate;
    }
