...
//        }
    ...
    public User findOneByStudentId( Long id){
        log.info("find One By Student Id", id);
        return userRepository.findOneByStudentId(id);
    }

    public String getIdByNotificationId(Long id){
        log.debug("Request to getIdByNotificationId", id);
        return  userRepository.getIdByNotificationId(id);
    }
    ...
    public Boolean checkIsAdmin(String user_id) {
        if (userRepository.countAdmin(user_id) == 1) {
            return true;
        } else {
            return false;
        }
